...
template <typename T>
using LockFreeStack  = LockFreeStackWithReferenceCount<T>;
...
void thread_consumer(void* data) {
  ...
  bool skip = false;
  std::shared_ptr<StackValue> stack_value;
  while (true) {
    if ((stack_value = global_conf->stack.Pop())) {
      // Check value
...
void thread_producer(void* data) {
  ...
  for (int64_t i = 0; i < global_conf->loop_times; i++) {
    ...
    global_conf->stack.Push(stack_value);
  }
...
int main(const int argc, char** argv) {
  ...
  int64_t count = available / sizeof(LockFreeStack<StackValue>::Node) / producer_count;
  fprintf(stdout, "size of CountedNodePtr[%zd]bytes\n", sizeof(LockFreeStack<StackValue>::CountedNodePtr));
  fprintf(stdout, "size of Node[%zd]bytes\n", sizeof(LockFreeStack<StackValue>::Node));
